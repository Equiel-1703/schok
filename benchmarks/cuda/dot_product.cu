...
__global__ void map_2kernel(float *a1, float *a2, float *a3, int size, float (*f)(float, float))
{
...
__global__ void reduce_kernel(float *a, float *ref4, float (*f)(float, float), int n)
{
...
__device__ float anonymous_mult(float a, float b)
{
    return ((a * b));
}
...
__device__ float anonymous_sum(float a, float b)
{
    return ((a + b));
}
